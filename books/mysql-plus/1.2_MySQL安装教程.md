...
> [官方安装教程](https://dev.mysql.com/doc/mysql-getting-started/en/)
...
![image-20210112201233332](https://ning-wang.oss-cn-beijing.aliyuncs.com/blog-imags/image-20210112201233332.png)

百度网盘

链接：https://pan.baidu.com/s/1eNgDu_D6NL6tyFrPSdDC0Q 
提取码：vycr 
...
lsof -i:3306 # 查看mysql 端口
mysql # 查看mysql命令
rpm -qa | grep -i mysql # 查看有没有装过mysql
```
...
![image-20210112204558399](https://ning-wang.oss-cn-beijing.aliyuncs.com/blog-imags/image-20210112204558399.png)
...
| 路径                | 解释                      | 备注                           |
| ------------------- | ------------------------- | ------------------------------ |
| `/var/lib/mysql/`   | mysql数据库文件的存放路径 | `/var/lib/mysql` 数据库文件    |
| `/usr/share/mysql`  | 配置文件目录              | `mysql.server`命令及配置文件   |
| `/usr/bin`          | 相关命令                  | `mysqladmin` `mysqldump`等命令 |
...
```mysql
use mysql;
update user set password=password('bdilab@1308') where `User`='root'; # 设置密码
...
`MySQL` 配置文件位于`/etc/my.conf`

默认配置文件位置

* 低版本 `/usr/share/mysql/my-huge.cnf`
* 高版本 `/usr/share/mysql/my-default.cnf`
...
> 更多详细设置参考[再见乱码：5分钟读懂MySQL字符集设置](https://www.cnblogs.com/chyingp/p/mysql-character-set-collation.html)
...
默认`Linux`的`MySQL`文件存储在`/var/lib/mysql`
