...
在Python中，可以使用切片（slice）来访问序列（如字符串、列表、元组）中的子序列。切片的语法是使用方括号[]，并在方括号内指定起始位置、终止位置和步长，中间用冒号:隔开。具体实现方法如下：

1. 切片语法：`seq[start:stop:step]`，其中start表示起始位置，stop表示终止位置，step表示步长。如果省略start，则默认从0开始；如果省略stop，则默认到序列的末尾；如果省略step，则默认为1。

2. 使用正整数作为索引，表示从序列的左侧开始计数，第一个元素的索引为0，最后一个元素的索引为len(seq)-1。

3. 使用负整数作为索引，表示从序列的右侧开始计数，最后一个元素的索引为-1，倒数第二个元素的索引为-2，以此类推。

4. 切片操作返回的是一个新的序列，不会修改原有的序列。
...
```
### 原始字符串阻止转义
原始字符阻止转义其实就是在字符串的第一引号的前面，关闭转义机制，结果就是python会将反斜杠组为字面量来保持，完全就像输入的那样。
```python
myfile = opem(r'C:\Users\王宁\zml\chrome\chrome.exe'，'w')
```